...
func _on_slider_value_changed(new_value: float) -> void:
	if variable == null:
		return
	variable.call("set_value", _get_variable_value_from_slider(new_value), self)
...
func _sync_slider_range(clamp_enabled: Variant = null, new_min_value: Variant = null, new_max_value: Variant = null) -> void:
	if variable == null:
		return
...
func _sync_slider_step() -> void:
	if variable is IntVariable:
		step = 1.0
		return
	step = _default_step

func _get_variable_value_from_slider(new_value: float) -> Variant:
	if variable is IntVariable:
